...
export function BankingProvider({ children }: { children: React.ReactNode }) {
  ...
  // Check if we have real user accounts from the database (stored at login)
  ...
  const [transactions, setTransactions] = useState<Transaction[]>(() => {
    // For DB users, start empty - transactions load from Supabase real-time sync
    ...
    // Demo/default data for non-DB users
    return [
    {
      id: "tx-alsco-001",
        ...
        notificationsChannel = supabase
          ...
          .on(
            ...
            (payload: any) => {
              ...
              // Show push notification via Service Worker (works in background too)
              if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
                navigator.serviceWorker.controller.postMessage({
                  type: 'SHOW_NOTIFICATION',
                  title: newNotif.title,
                  body: newNotif.message,
                  notificationType: newNotif.type || 'info',
                  amount: newNotif.data?.amount,
                  tag: `chase-${newNotif.id}`,
