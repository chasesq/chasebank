...
function sendBrowserNotification(options: {
  ...
  try {
    const { recipientName, amount, accountName } = options

    if ('Notification' in window && Notification.permission === 'granted') {
      const notification = new Notification('Chase - Funds Received', {
        body: `$${amount.toFixed(2)} has been added to your ${accountName}`,
        icon: '/logo.png',
        badge: '/badge.png',
        tag: 'chase-transfer-alert',
    ...
    console.error('[v0] Browser notification error:', error)
    return {
      channel: 'browser',
      success: false,
      error: 'Failed to show browser notification',
    }
  }
}
