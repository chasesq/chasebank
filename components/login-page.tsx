...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleSignIn = async () => {
    ...
    try {
      // Real-time authentication with retry logic
      let authAttempts = 0
      const maxAttempts = 3
      let authSuccess = false

      while (authAttempts < maxAttempts && !authSuccess) {
        try {
          // Simulate real-time auth call with timeout
          const authPromise = new Promise<boolean>((resolve) => {
            setTimeout(() => {
              ...
              resolve(!!isDefaultUser || !!storedUser)
            }, 600)
          })
          ...
          authSuccess = await Promise.race([authPromise, timeoutPromise])
          break
        } catch (error) {
          authAttempts++
      ...
      if (settingsEnforcer && settingsEnforcer.check2FA()) {
        ...
        const twoFACode = generateToken()
        setModalView("2fa-verify")
        setIsLoading(false)
        toast({
          title: "Verification Required",
          description: `A code has been sent to ${appSettings?.twoFactorPhone || "your phone"} (Code: ${twoFACode})`,
        })
        return
      }

      // Real-time biometric check
      ...
      const storedUserData = storedUsers.find((u) => u.username === username)
      const displayName = storedUserData ? storedUserData.firstName : defaultUserProfile.name

      // Real-time security check
      if (settingsEnforcer && settingsEnforcer.shouldShowAlert("login")) {
        console.log("[v0] Showing security alert")
        toast({
