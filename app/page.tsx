...
export default function Page() {
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      // Load stored accounts into the banking context
      if (loggedIn) {
        try {
          const storedAccounts = localStorage.getItem("chase_user_accounts")
          if (storedAccounts) {
            const accounts = JSON.parse(storedAccounts)
            console.log("[v0] Loaded", accounts.length, "accounts from session")
          }
        } catch (e) {
          console.error("[v0] Error loading accounts:", e)
        }
      }
  ...
  useEffect(() => {
    ...
    const isDemo = localStorage.getItem("chase_is_demo") === "true"
    ...
    const welcomeTimer = setTimeout(() => {
      const demoNotice = isDemo ? " (Demo Mode)" : ""
      toast({
        title: `Welcome back, ${getUserFirstName()}!${demoNotice}`,
