...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'initiate') {
      ...
      const { otp: generatedOtp, expiresAt } = await generateAndStoreOTP(
        adminId,
        'admin_transfer'
      )
    ...
    if (action === 'confirm') {
      ...
      // Verify OTP
      const otpValid = await verifyOTP(adminId, otp, 'admin_transfer')
      if (!otpValid) {
        return NextResponse.json(
      ...
      if (account) {
        newBalance = account.balance + transfer.amount
