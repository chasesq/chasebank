...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('[v0] Supabase client initialization failed:', clientError)
    ...
    if (action === 'login') {
      ...
      try {
        // Find user by email or name (support both email and username login)
        let query = supabase
          .from('users')
          .select('*')
        
        // Try exact match first (handles both email and username)
        ...
        if (!users || users.length === 0) {
          console.log(`[v0] User not found for: ${email}`)
          return NextResponse.json(
        ...
        const user = users[0]
        console.log(`[v0] User found: ${user.name}, ID: ${user.id}`)

        // Verify password
        const passwordValid = await verifyPassword(password, user.password_hash)
        if (!passwordValid) {
          console.log(`[v0] Password verification failed for user: ${user.name}`)
          return NextResponse.json(
            { error: 'Invalid credentials' },
            { status: 401 }
          )
        ...
        console.log(`[v0] Password verification successful for user: ${user.name}`)
        ...
        // If Supabase is unreachable, log the error and return a helpful message
        console.error(`[v0] Login error:`, loginError instanceof Error ? loginError.message : String(loginError))
        
        if (isNetworkError(loginError)) {
          return NextResponse.json(
            { 
              error: 'Service temporarily unavailable',
              suggestion: 'Please check your internet connection and try again. Demo login is available.',
              status: 503
            },
            { status: 503 }
          )
  ...
  logSupabaseError('Auth Route', error)
